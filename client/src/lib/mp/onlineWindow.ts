...
import { ONLINE_WINDOW_CLOSED } from './messages';

const jq = (jqueryProxy).default || jqueryProxy;

var OnlineWindow = ModalWindow(function() {
...
OnlineWindow.prototype.close = function(data) {
  data = data || {cancelled: true};
  this._emitEvent(ONLINE_WINDOW_CLOSED, data);
  this._toggleDisplay();
};
...
OnlineWindow.prototype.open = function(budgetData) {
  var i, elem;
