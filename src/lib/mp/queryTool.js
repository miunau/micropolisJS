...
QueryTool.prototype.classifyPopulationDensity = function(x, y, blockMaps) {
  ...
  if (Config.debug || Config.queryDebug)
    jq('#queryDensityRaw').text(density);
  density = density >> 6;
  density = density & 3;
  jq('#queryDensity').text(Text.densityStrings[density]);
};
...
QueryTool.prototype.classifyLandValue = function(x, y, blockMaps) {
   ...
   if (Config.debug || Config.queryDebug)
     jq('#queryLandValueRaw').text(landValue);
...
QueryTool.prototype.classifyCrime = function(x, y, blockMaps) {
  ...
  if (Config.debug || Config.queryDebug)
    jq('#queryCrimeRaw').text(crime);
  crime = crime >> 6;
  crime = crime & 3;
  jq('#queryCrime').text(Text.crimeStrings[crime]);
};
...
QueryTool.prototype.classifyPollution = function(x, y, blockMaps) {
  ...
  if (Config.debug || Config.queryDebug)
    jq('#queryPollutionRaw').text(pollution);
  pollution = pollution >> 6;
  pollution = pollution & 3;
  jq('#queryPollution').text(Text.pollutionStrings[pollution]);
};
...
QueryTool.prototype.classifyRateOfGrowth = function(x, y, blockMaps) {
  ...
  if (Config.debug || Config.queryDebug)
    jq('#queryRateRaw').text(rate);
  rate = rate >> 6;
  rate = rate & 3;
  jq('#queryRate').text(Text.rateStrings[rate]);
};
...
QueryTool.prototype.classifyDebug = function(x, y, blockMaps) {
  if (!Config.debug && !Config.queryDebug)
    return;
  jq('#queryFireStationRaw').text(blockMaps.fireStationMap.worldGet(x, y));
  jq('#queryFireStationEffectRaw').text(blockMaps.fireStationEffectMap.worldGet(x, y));
  jq('#queryPoliceStationRaw').text(blockMaps.policeStationMap.worldGet(x, y));
  jq('#queryPoliceStationEffectRaw').text(blockMaps.policeStationEffectMap.worldGet(x, y));
  jq('#queryTerrainDensityRaw').text(blockMaps.terrainDensityMap.worldGet(x, y));
  jq('#queryTrafficDensityRaw').text(blockMaps.trafficDensityMap.worldGet(x, y));
  jq('#queryComRateRaw').text(blockMaps.cityCentreDistScoreMap.worldGet(x, y));
};
...
QueryTool.prototype.classifyZone = function(x, y) {
  ...
  jq('#queryZoneType').text(Text.zoneTypes[index]);
};
...
QueryTool.prototype.doTool = function(x, y, blockMaps) {
  ...
  if (Config.debug || Config.queryDebug) {
    ...
    jq('#queryTile').text([x,y].join(', '));
    jq('#queryTileValue').text(tile.getValue());
    jq('#queryTileBurnable').text(tile.isCombustible() ? '\u2714' : '\u2718');
    jq('#queryTileBulldozable').text(tile.isBulldozable() ? '\u2714' : '\u2718');
    jq('#queryTileCond').text(tile.isConductive() ? '\u2714' : '\u2718');
    jq('#queryTileAnim').text(tile.isAnimated() ? '\u2714' : '\u2718');
    jq('#queryTilePowered').text(tile.isPowered() ? '\u2714' : '\u2718');
    jq('#queryTileZone').text(tile.isZone() ? '\u2714' : '\u2718');
  }
