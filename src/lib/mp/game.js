...
Game.prototype.revealControls = function() {
 jq('.initialHidden').each(function(e) {
   jq(this).removeClass('initialHidden');
 });
...
Game.prototype.handleTool = function(data) {
  ...
  switch (tool.result) {
    case tool.TOOLRESULT_NEEDS_BULLDOZE:
      jq('#toolOutput').text(Text.toolMessages.needsDoze);
      break;

    case tool.TOOLRESULT_NO_MONEY:
      jq('#toolOutput').text(Text.toolMessages.noMoney);
      break;

    default:
      jq('#toolOutput').html('Tools');
  }
};
...
var tick = function() {
  ...
  if (!this.simulation.isPaused() && !jq('#tooSmall').is(':visible')) {
    // Run the sim
...
var debugAnimate = function() {
  ...
  if (elapsed > this.lastElapsed && this.frameCount > 0) {
    jq('#fpsValue').text(Math.floor(this.frameCount/elapsed));
